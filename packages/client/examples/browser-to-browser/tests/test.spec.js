...
const sendBtn = "#send";
const output = "#output";
const listeningAddresses = "#invites";
...
test.describe("browser to browser example:", () => {
  ...
  test("should connect to a relay node", async ({ page, context }) => {
    ...
    const invite = await getInvite(page, relayNodeAddr);

    // load second page and use `peer` as the connectAddr
    const pageTwo = await context.newPage();
    await pageTwo.goto(url);
    await joinInvite(pageTwo, relayNodeAddr, invite);
...
async function getInvite(page, address) {
  await page.fill(connectAddr, address);
  await page.click(connectBtn);

  const outputLocator = page.locator(output);
  await expect(outputLocator).toHaveText(/Got invite/);

  const invite = await page.textContent(listeningAddresses);

  return invite;
}

async function joinInvite(page, address, invite) {
  // add the go libp2p multiaddr to the input field and submit
  await page.fill(connectAddr, address);
  await page.fill(inviteToken, invite);
  await page.click(connectBtn);
