...
export function ShareLink() {
  ...
  effect(() => {
    if (joinLink.value) {
      QRCode.toDataURL(joinLink.value, { width: 194 }).then(
        (val: any) => (qr.value = val)
      );
    }
  });
  ...
  }, [joinLink.value, hasShare, linkShared]);

  return (
    <div className="self-center max-w-xs flex flex-col align-items-center">
      <h3 className="font-bold text-lg mb-2">
        Share this link to DM discretely
      </h3>

      <div className="space-2 pb-0">
        <div className="form-control">
          <label className="input-group input-group-sm">
            ...
            {hasShare ? (
              <button
                className="opacity:2 btn btn-sm btn-circle self-center"
                onClick={onShare}
              >
                <svg
      ...
      <div className="self-center rounded-lg overflow-hidden">
        {qr.value ? (
          ...
          <div className="animate-pulse dark:bg-base-100 bg-slate-200 w-48 h-48" />
        )}
      </div>
    </div>
  );
