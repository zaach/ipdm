...
async function spawnRelay() {
  ...
  relayNode.addEventListener("self:peer:update", (event) => {
    console.log("self:peer:update", event.detail, event.detail.peer.addresses);
    const addrs = relayNode.getMultiaddrs();
    console.log("self:peer:update getMultiaddrs", addrs);
  });


  const relayNodeAddr = relayNode.getMultiaddrs()[0].toString()
  //const relayNodeAddr = "/ip4/192.168.1.15/tcp/4002/ws/p2p/12D3KooWPdSEq6e2bVvSQWWkJ5h9ukJ8mAd5CNjUVwG6uF7fzCn8"
  //const relayNodeAddr = "/ip4/127.0.0.1/tcp/4002/ws/p2p/12D3KooWPdtkVjN7U7TPMQNL1ztSJWR3MyzmctscPienEDvVDnhi"
  //const relayNodeAddr = "/dnsaddr/bootstrap.libp2p.io/ws/p2p/QmNnooDu7bfjPFoTZYxMNLWUQJyrVwtbZg5gBMjTezGAJN"
...
async function dialRelay (page, address) {
  ...
  console.log('listening address:', multiaddrs)

  console.log('Relayed address:', address, addr)
